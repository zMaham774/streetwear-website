...
gsap.set(["[data-ch-crumb]", "[data-ch-heading]"], { opacity: 0, y: 20 });

const contactHeroTL = gsap.timeline({ delay: 0.5 });
contactHeroTL
    .to("[data-ch-crumb]", { opacity: 1, y: 0, duration: 0.5, ease: "power3.out" })
    .to("[data-ch-heading]", { opacity: 1, y: 0, duration: 0.8, ease: "power3.out" }, "-=0.2");

gsap.from("#nav", { y: -20, opacity: 0, duration: 0.8, delay: 0.3, ease: "power3.out" });

/* FORM + INFO SPLIT ANIMATIONS */

gsap.set("[data-cf-form]", { opacity: 0, x: -30 });
gsap.set("[data-cf-info]", { opacity: 0, x: 30 });

gsap.to("[data-cf-form]", {
    scrollTrigger: {
